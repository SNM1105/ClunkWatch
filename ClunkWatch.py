...
def start_ap_mode():
    ...
    ap.active(True)
    ap.config(essid="PicoChat", password="picopico123")
    print("=== AP MODE STARTED ===")
    print("SSID: PicoChat")
    print("Password: picopico123")
    while not ap.active():
        time.sleep_ms(500)
    ip = ap.ifconfig()[0]
...
def start_server():
    ...
    try:
        ...
        print("Server listening on port", MSG_PORT)
    except Exception as e:
        print("Server error:", e)
        server_sock = None

def check_incoming():
    global msg_received, PEER_IP
...
def wifi_update(now):
    lcd_clear_print(0, "AP Active!")
    lcd_clear_print(1, "PicoChat ready")

# ====================== STARTUP ======================
time.sleep_ms(500)
lcd_init()
lcd_clear_print(0, " Heart Watch ")
lcd_clear_print(1, " AP Receiver ")
time.sleep_ms(1500)

lcd_clear_print(0, "Starting AP...")
lcd_clear_print(1, "Please wait...")
start_ap_mode()
wifi_mode = "ap_ready"

last_screen_content = ("", "")
print("Receiver Ready - AP and server running")
